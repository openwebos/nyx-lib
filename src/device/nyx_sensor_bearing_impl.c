...
nyx_error_t nyx_sensor_bearing_get_location(nyx_device_handle_t handle, nyx_sensor_bearing_location_t* location)
{
    nyx_device_t* d = (nyx_device_t*)handle;
    CHECK_DEVICE(d);
    nyx_bearing_get_location_function_t f = LOOKUP_METHOD(d, NYX_SENSOR_BEARING_GET_LOCATION_MODULE_METHOD);
    if (f)
        return f(d,location);
    else
