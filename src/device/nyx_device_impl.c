...
static int32_t _static_scan_directory(const char* directory_str, const char* type_str, nyx_device_filter_t filter, struct nyx_device_iterator *iterator_in_out_ptr)
{
	struct dirent *dp;
	int32_t count = 0;
		...
		if (filename_str_length > prefix_length + type_str_length + suffix_length){
			...
			if (0 == strncmp(NYX_MODULE_PREFIX, dp->d_name, prefix_length)) {
				...
				if (0 == strncmp(type_str, dp->d_name + prefix_length, type_str_length)) {
					...
					if (0 == strncmp(NYX_MODULE_SUFFIX, dp->d_name + filename_str_length - suffix_length, suffix_length)) {
						// we have the right suffix and prefix.
						const char* id_str = (char*)strndup( dp->d_name + prefix_length +  type_str_length, filename_str_length - prefix_length - type_str_length - suffix_length);
						iterator_in_out_ptr->list = g_slist_prepend(iterator_in_out_ptr->list, (gpointer)id_str);
						count++;
					}
				}
			}
...
nyx_error_t nyx_device_get_iterator(nyx_device_type_t type, nyx_device_filter_t filter, nyx_device_iterator_handle_t *iterator_out_ptr)
{
	...
	struct nyx_device_iterator *i = (struct nyx_device_iterator*)calloc(sizeof(struct nyx_device_iterator),1);
...
nyx_error_t nyx_device_iterator_get_next_id(nyx_device_iterator_handle_t iterator, nyx_device_id_t *id_out_ptr)
{
...
nyx_error_t nyx_module_register_method(nyx_instance_t instance, nyx_device_t* device_in_ptr, module_method_t method, const char* symbol_str)
{
	...
	if (NULL == device_in_ptr->method_hash_table)
		device_in_ptr->method_hash_table = g_hash_table_new (g_direct_hash, g_direct_equal);

	g_hash_table_replace ((GHashTable*)device_in_ptr->method_hash_table, GINT_TO_POINTER(method), f);
...
nyx_error_t nyx_device_set_operating_mode(nyx_device_handle_t handle, nyx_operating_mode_t mode)
{
	nyx_device_t* d = (nyx_device_t*)handle;
	CHECK_DEVICE(d);
	nyx_set_operating_mode_function_t f = LOOKUP_METHOD(d, NYX_SET_OPERATING_MODE_MODULE_METHOD);
	if (f)
		return f(d, mode);
	else
		return NYX_ERROR_NOT_IMPLEMENTED;
...
nyx_error_t nyx_device_set_report_rate(nyx_device_handle_t handle, nyx_report_rate_t rate)
{
	nyx_device_t* d = (nyx_device_t*)handle;
	CHECK_DEVICE(d);
