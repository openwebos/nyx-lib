...
nyx_error_t nyx_sensor_orientation_get_threshold(nyx_device_handle_t handle, nyx_sensor_orientation_threshold_t* threshold)
{
    nyx_device_t* d = (nyx_device_t*)handle;
    CHECK_DEVICE(d);
    nyx_orientation_get_threshold_function_t f = LOOKUP_METHOD(d, NYX_SENSOR_ORIENTATION_GET_THRESHOLD_MODULE_METHOD);
    if (f)
