...
nyx_error_t nyx_touchpanel_get_active_scan_rate(nyx_device_handle_t handle, uint32_t *active_scan_rate_out_ptr)
{
    nyx_device_t* d = (nyx_device_t*)handle;
    CHECK_DEVICE(d);
...
nyx_error_t nyx_touchpanel_set_idle_scan_rate(nyx_device_handle_t handle, uint32_t idle_scan_rate_in)
{
    nyx_device_t* d = (nyx_device_t*)handle;
    CHECK_DEVICE(d);
...
nyx_error_t nyx_touchpanel_get_mode(nyx_device_handle_t handle, int32_t *mode_mask_out_ptr)
{
    nyx_device_t* d = (nyx_device_t*)handle;
    CHECK_DEVICE(d);
    nyx_get_int_function_t f = LOOKUP_METHOD(d,  NYX_TOUCHPANEL_GET_MODE_MODULE_METHOD);
    if (f)
